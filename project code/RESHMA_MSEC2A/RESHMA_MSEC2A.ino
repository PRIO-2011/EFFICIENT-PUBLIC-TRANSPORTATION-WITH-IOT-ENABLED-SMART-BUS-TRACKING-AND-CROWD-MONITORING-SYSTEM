...
void setup() {
    ...
    randomSeed(analogRead(0)); // Use noise from analog pin to seed
}
...
void loop() {
    ...
    // Generate random estimated time (in minutes)
    int eta = random(1, 10); // ETA between 1 to 9 minutes

    // Display crowd status and ETA based on switch conditions
    lcd.setCursor(0, 1);
    if (RF1a == HIGH) {
        lcd.print("Very Low ");
        lcd.print(eta);
    ...
    delay(2000); // Delay to allow the user to read LCD
}
